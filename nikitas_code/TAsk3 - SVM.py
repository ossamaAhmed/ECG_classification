...
def getData():
    #Input file names
    x_file="X_train_extracted.csv"
    y_file="y_train.csv"
    x_test_file="X_test_extracted.csv"
    ...
    df22 = pd.read_csv(x_test_file2, header=0)
    ind = range(3411)


    # Extract, from the data frames, the 2D arrays of values
    x_train = np.concatenate( (df0.values, df02.values),axis=1)
    y_train = df1.values[:, 1]
    x_test = np.concatenate( (df2.values, df22.values),axis=1)
...
def fitdata(x_train, y_train,):
    ...
    reg = svm.SVC(C=1.0, cache_size=200, coef0=0.0,
                decision_function_shape='ovo', degree=3, gamma='scale', kernel='sigmoid',
                max_iter=-1, probability=False, random_state=None, shrinking=True,
                tol=0.001, verbose=False )
    if GRID:
        grid_params=[{'C': [50, 100,150], 'kernel': ['rbf'],'decision_function_shape': ['ovr']},
                     {'C': [50, 100, 150], 'kernel': ['rbf'], 'decision_function_shape': ['ovo']}]
        ...
        reg= svm.SVC(C=50,decision_function_shape='ov5',gamma='scale', kernel='rbf')
